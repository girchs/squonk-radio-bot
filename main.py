...
# Sample songs (replace with real file_ids or file paths)
songs = [
...
async def play_cmd(message: types.Message):
    ...
    await message.reply_audio(open(song["file"], "rb"), caption=song["comment"], reply_markup=get_keyboard())

@dp.callback_query_handler(lambda c: c.data in ["next", "replay"])
async def callback_play(call: types.CallbackQuery):
    song = random.choice(songs) if call.data == "next" else songs[0]  # replay first one
    await bot.send_audio(call.message.chat.id, open(song["file"], "rb"), caption=song["comment"], reply_markup=get_keyboard())
    await call.answer()
